...
namespace FoodHygieneRatingDotNet.Tests
{
    ...
    {
        public class GetAsyncMethod : SchemeTypeServiceTestsBase
        {
            [Test]
            public async Task ProvidesCorrectApiUrl()
            {
                ...
                // Assert
                var expected = $"{HttpConstants.FhrHttpProtocol}://{HttpConstants.FhrBaseApiUrl}/SchemeTypes";
            ...
            [TestCase("{ schemeTypes: [{ schemeTypeName: \"Foo\" }] }", "Foo")]
            [TestCase("{ schemeTypes: [{ schemeTypeName: \"Bar\" }] }", "Bar")]
            public async Task SerialisesContentStringToCorrectObject(string content, string expectedName)
            {
                ...
                // Assert
                Assert.AreEqual(response.Response.Data[0].SchemeTypeName, expectedName);
            }
