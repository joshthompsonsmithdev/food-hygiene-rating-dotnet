...
{
    public class SortOptionServiceTests
    {
        public class GetAsyncMethod : SortOptionServiceTestsBase
        {
            [Test]
            public async Task ProvidesCorrectApiUrl()
            {
                ...
                // Assert
                var expected = $"{HttpConstants.FhrHttpProtocol}://{HttpConstants.FhrBaseApiUrl}/SortOptions";
            ...
            [TestCase("{ sortOptions: [{ sortOptionName: \"Foo\" }] }", "Foo")]
            [TestCase("{ sortOptions: [{ sortOptionName: \"Bar\" }] }", "Bar")]
            public async Task SerialisesContentStringToCorrectObject(string content, string expectedName)
            {
                ...
                // Assert
                Assert.AreEqual(response.Response.Data[0].SortOptionName, expectedName);
            }
